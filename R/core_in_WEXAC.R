...
#' PANORAMA object constructor from META table
#'
#'
#'
...
#'
#' @examples
panorama_PANORAMA <- function(META, genome = NULL, geneAnnot = NULL, nCores = 1, verb = FALSE){
    if(hasDups(META$id)){
    ...
    PANORAMA <- list(META = META,
                  DATA = DTL,
                  RES  = NULL)
    if(verb){cat("PANORAMA object generated. \n")}
    return(PANORAMA)
}
...
bisGenome <- function(fastaGenome, outFile = "auto"){
    if(outFile == "auto"){
        ...
        outName <- paste0(strsplit(fastaGenome, split = "/") %>% unlist %>% utils::tail(1),
                          ".bis")
        outFile <- file.path(getwd(), "PANORAMA_tmpDir", outName)
    }
    gen <- Biostrings::readDNAStringSet(fastaGenome)
    gen_r <- lapply(gen, function(x){
        stringr::str_replace_all(as.character(x), pattern = "C", replacement = "T")
    })
...
mkTranscriptome <- function(fastaGenome, bedAnnotation, outFile = "auto", nCores){
    ...
    if(outFile == "auto"){
        mkTmpDir()
        fileN <- strsplit(fastaGenome, split = "/") %>% unlist %>% utils::tail(1)
        outFile <- file.path("PANORAMA_tmpDir", paste0(fileN, ".txOme"))
    }
...
mkBedFromFastaTxOme <- function(fastaTxOme, outFile = "auto"){
    ...
    if(max(Biostrings::width(fa)) > 50000){
        warning("Maximum sequence length exceeded 50'000, make sure sequences represent transcripts. This is just a warning.")
    }
    if(outFile == "auto"){
        mkTmpDir()
        fileN <- strsplit(fastaTxOme, split = "/") %>% unlist %>% utils::tail(1)
        outFile <- file.path("PANORAMA_tmpDir", paste0(fileN, ".bed"))
    }
