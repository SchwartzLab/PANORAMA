...
#' Add all default metrics for Pan-Mod-seq
#'
...
#' @export
#'
add_default_metrics <- function(PANORAMA, nCores = 1){
    PANORAMA %>%
        ...
        add_MRD("Mock.TGIRT", "Mock.SSIII", nCores = nCores) %>%
        add_MRD("Mock.RTHIV", "Mock.TGIRT", nCores = nCores) %>%
        ...
        add_SR1bpDS("Mock.SSIII", nCores = nCores) %>%
        ...
        add_SRlog2FCh1bpDS("Mock.SSIII", "Dimroth.SSIII", nCores = nCores)
}

# add_Y_metrics <- function(PANORAMA){
#     PANORAMA %>%
...
#         add_DRD("BIDseq.SSIII", "Mock.SSIII")
# }
#
# add_Nm_metrics <- function(PANORAMA){
#     PANORAMA %>%
...
# }
#
# add_ac4C_metrics <- function(PANORAMA){
#     PANORAMA %>%
...
# }
#
# add_m1A_metrics <- function(PANORAMA){
#     PANORAMA %>%
...
# }
#
# add_m7G_metrics <- function(PANORAMA){
#     PANORAMA %>%
...
# }
#
# add_m5C_metrics <- function(PANORAMA){
#     PANORAMA %>%
...
# }
#
# add_m3U_metrics <- function(PANORAMA){
#     PANORAMA %>%
#         add_MRD("Mock.TGIRT", "AlkBmix.TGIRT")
# }
#
# add_Nm_MgR_metrics <- function(PANORAMA){
#     PANORAMA %>%
#         add_ER1bpDS("MgR") %>%
#         add_ER1bpDS("MgR-OED") %>%
#         add_ER1bpDS("Mock") %>%
...
# }
#
# add_m66A_metrics <- function(PANORAMA){
#     PANORAMA %>%
...
#'
#' @examples
hlp_start_CALLS <- function(PANORAMA){
    RNAmods <- check_whichRNAmods(PANORAMA)
    PANORAMA$CALLS <- lapply(seq_along(unique(PANORAMA$META$set)), function(x) {
        lapply(RNAmods, function(x) NULL) %>% magrittr::set_names(RNAmods)
    })
    names(PANORAMA$CALLS) <- unique(PANORAMA$META$set)
    PANORAMA
}

#' Assign scores to respective RNAmods in PANORAMA$CALLS
#'
#' @param PANORAMA list. PANORAMA object as output by \code{\link{panorama_PANORAMA}}.
...
#'
#' @examples
hlp_assign_scores <- function(PANORAMA, RNAmod, scores, onNucs = c("A", "C", "G", "T")){
    if(any(scores %in% unique(PANORAMA$RES$metric))){
        tmp <- PANORAMA$RES %>%
            tidyr::pivot_wider(names_from = metric, values_from = score) %>%
            data.table::data.table()
        scoresInTable <- intersect(scores, colnames(tmp))
        selVars <- c(panorama_baseCols, scoresInTable)
        ...
        if(length(scoresNotInTable) > 0){
            warning(paste0(scoresNotInTable, collapse = " "), " metric(s) are not ",
                    "present in PANORAMA$RES$metric, and were not added to CALLS. ",
                    "This is just a warning.")
        }
        tmp <- split(tmp, tmp$set)
        for(iN in names(tmp)){
            PANORAMA$CALLS[[iN]][[RNAmod]] <- tmp[[iN]]
        }
        return(PANORAMA)
    }else{
...
#'
#' @examples
panorama_assignScores <- function(PANORAMA){
    hlp_start_CALLS(PANORAMA) %>%
        hlp_assign_scores("Y", PANORAMA::Y_metrics) %>%
        hlp_assign_scores("Am", PANORAMA::Nm_metrics, "A") %>%
        hlp_assign_scores("Um", PANORAMA::Nm_metrics, "T") %>%
        hlp_assign_scores("Gm", PANORAMA::Nm_metrics, "G") %>%
        hlp_assign_scores("Cm", PANORAMA::Nm_metrics, "C") %>%
        hlp_assign_scores("m5C", PANORAMA::m5C_metrics) %>%
        hlp_assign_scores("ac4C", PANORAMA::ac4C_metrics) %>%
        hlp_assign_scores("m1A", PANORAMA::m1A_metrics) %>%
        hlp_assign_scores("m7G", PANORAMA::m7G_metrics) %>%
        hlp_assign_scores("m3U", PANORAMA::m3U_metrics) %>%
        hlp_assign_scores("m66A", PANORAMA::m66A_metrics) %>%
        hlp_assign_scores("m1acp3Y", PANORAMA::m1acp3Y_metrics) %>%
        hlp_assign_scores("D", PANORAMA::D_metrics) %>%
        hlp_assign_scores("m1G", PANORAMA::m1G_metrics) %>%
        hlp_assign_scores("m3C", PANORAMA::m3C_metrics) %>%
        hlp_assign_scores("m22G", PANORAMA::m22G_metrics)
}
# BASE FUNCTIONS ###############################################################
...
#' of group_A over that of group_B per set of samples in a PANORAMA object.
#'
#' @param PANORAMA list. PANORAMA object as output by \code{\link{panorama_PANORAMA}}
#' @param group_A character. Name of group to be compared as found in PANORAMA$META$group
#' @param group_B character. Name of group to be compared as found in PANORAMA$META$group
#' @param newColName character. Name of calculated metric to be stored in PANORAMA$RES,
...
#' @export
#'
add_ERlog2FCh1bpDS=function(PANORAMA, group_A, group_B, newColName = "auto",
                            onNucs = c("A", "C", "G", "T"), minCov = 50, nCores = 1){
    if(newColName == "auto"){
        newColName <- paste("ERlog2FCh1bpDS", group_A, group_B, sep = "_")
    }
    sets <- intersect(PANORAMA$META[PANORAMA$META$group == group_A,]$set,
                      PANORAMA$META[PANORAMA$META$group == group_B,]$set)
    if(length(sets) == 0){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        id_B <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_B,]$id
        DT_A <- add_EndRate1bpDS(PANORAMA$DATA[[id_A]], minCov = minCov)
        DT_B <- add_EndRate1bpDS(PANORAMA$DATA[[id_B]], minCov = minCov)
    ...
    PANORAMA$RES <- hlpr_add_REScols(PANORAMA$RES, OUT)
    return(PANORAMA)
}
...
#' @export
#'
add_ERlog2FCh <- function(PANORAMA, group_A, group_B, newColName = "auto",
                          onNucs = c("A", "C", "G", "T"), minCov = 50, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        id_B <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_B,]$id
        DT_A <- add_EndRate(PANORAMA$DATA[[id_A]], minCov = minCov)
        DT_B <- add_EndRate(PANORAMA$DATA[[id_B]], minCov = minCov)
...
#' of group_B per set of samples in a PANORAMA object.
#'
#' @param PANORAMA list. PANORAMA object as output by \code{\link{panorama_PANORAMA}}
#' @param group_A character. Name of group to be compared as found in PANORAMA$META$group
#' @param group_B character. Name of group to be compared as found in PANORAMA$META$group
...
#' @export
#'
add_SRD <- function(PANORAMA, group_A, group_B, newColName = "auto",
                    onNucs = c("A", "C", "G", "T"), minCov = 50, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        id_B <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_B,]$id
        DT_A <- add_StartRate(PANORAMA$DATA[[id_A]], minCov = minCov)
        DT_B <- add_StartRate(PANORAMA$DATA[[id_B]], minCov = minCov)
...
#' @export
#'
add_SRD1bpDS <- function(PANORAMA, group_A, group_B, newColName = "auto",
                         onNucs = c("A", "C", "G", "T"), minCov = 50, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        id_B <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_B,]$id
        DT_A <- add_StartRate_1bpDS(PANORAMA$DATA[[id_A]], minCov = minCov)
        DT_B <- add_StartRate_1bpDS(PANORAMA$DATA[[id_B]], minCov = minCov)
...
#'
#' @examples
add_SRlog2FCh1bpDS <- function(PANORAMA, group_A, group_B, newColName = "auto",
...
#' Add 2-O-methylation score to PANORAMA object
#'
...
#'
#' @examples
add_2OmeScore <- function(PANORAMA,
                          lib_LowdNTPs,
                          lib_HighdNTPs,
                          newColName = "auto",
                          flankSize = 5,
                          perNuc_Zscaling = FALSE,
                          onNucs = c("A", "C", "G", "T"),
                          minCov = 50,
                          nCores = 1){
    if(newColName == "auto"){
    ...
    sets <- intersect(PANORAMA$META[PANORAMA$META$group == lib_LowdNTPs,]$set,
                      PANORAMA$META[PANORAMA$META$group == lib_HighdNTPs,]$set)
    if(length(sets) == 0){warning("No sets found with both ", lib_LowdNTPs,
                                  " and ", lib_HighdNTPs, " group labels.\n",
                                  newColName, " was not calculated.")
        return(PANORAMA)}
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == lib_LowdNTPs,]$id
        id_B <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == lib_HighdNTPs,]$id
        DT <- detect_2OmeScore(DT_low = PANORAMA$DATA[[id_A]],
                               DT_high = PANORAMA$DATA[[id_B]],
...
#'
#' @examples
add_MRD <- function(PANORAMA, group_A, group_B, newColName = "auto",
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        id_B <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_B,]$id
        DT_A <- PANORAMA$DATA[[id_A]] %>% add_diffNucToRefRatio(minReads = minReads)
        DT_B <- PANORAMA$DATA[[id_B]] %>% add_diffNucToRefRatio(minReads = minReads)
...
#'
#' @examples
add_MRDns <- function(PANORAMA, group_A, group_B, refNuc, misNuc, minReads = 30, nCores = 1){
...
#' Add cytidine persistance index to PANORAMA object
#'
...
#'
#' @examples
add_CytPer <- function(PANORAMA, group_A, group_B, newColName = "auto",
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        id_B <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_B,]$id
        DT_A <- PANORAMA$DATA[[id_A]] %>% detect_m5C(minReads = minReads)
        DT_B <- PANORAMA$DATA[[id_B]] %>% detect_m5C(minReads = minReads)
...
#'
#' @examples
add_CtoT_MRD <- function(PANORAMA, group_A, group_B, newColName = "auto",
                         onNucs = c("C"), minReads = 30, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        id_B <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_B,]$id
        DT_A <- PANORAMA$DATA[[id_A]] %>% detect_ac4C(minReads = minReads)
        DT_B <- PANORAMA$DATA[[id_B]] %>% detect_ac4C(minReads = minReads)
...
#'
#' @examples
add_DRD <- function(PANORAMA, group_A, group_B, newColName = "auto",
                    onNucs = c("A", "C", "G", "T"), minReads = 30, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        id_B <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_B,]$id
        DT_A <- PANORAMA$DATA[[id_A]] %>% deletion_rate(minReads = minReads)
        DT_B <- PANORAMA$DATA[[id_B]] %>% deletion_rate(minReads = minReads)
...
#'
#' @examples
add_ER1bpDS=function(PANORAMA, group_A, newColName = "auto",
                     onNucs = c("A", "C", "G", "T"), minCov = 50, nCores = 1){
    if(newColName == "auto"){
        newColName <- paste("ER1bpDS", group_A, sep = "_")
    }
    sets <- PANORAMA$META[PANORAMA$META$group == group_A,]$set
    if(length(sets) == 0){
...
#'
#' @examples
add_SR <- function(PANORAMA, group_A, newColName = "auto",
                   onNucs = c("A", "C", "G", "T"), minCov = 50, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        DT_A <- add_StartRate(PANORAMA$DATA[[id_A]])
...
#'
#' @examples
add_SR1bpDS <- function(PANORAMA, group_A, newColName = "auto",
                        onNucs = c("A", "C", "G", "T"), minCov = 50, nCores = 1){
    if(newColName == "auto"){
...
#'
#' @examples
add_ER <- function(PANORAMA, group_A, newColName = "auto",
                   onNucs = c("A", "C", "G", "T"), minCov = 50, nCores = 1){
    if(newColName == "auto"){
...
#' @param flankSize integer. Length of window to each side of position to
#' consider for metric calculation
#' @param minReads numeric. Minimum number of reads start/ends in window. Both
#' flanks and the queried site are summed to calculate this number.
#'
#' @return
#' @export
add_scoreA3p <- function(PANORAMA, group_A, newColName = "auto",
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        DT_A <- add_scoreA_3p(PANORAMA$DATA[[id_A]], flankSize = flankSize, minReads = minReads)
...
#' @export
#'
add_scoreA5p <- function(PANORAMA, group_A, newColName = "auto",
                         onNucs = c("A", "C", "G", "T"), minReads = 30,
                         flankSize = 6, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        DT_A <- add_scoreA_5p(PANORAMA$DATA[[id_A]], minReads = minReads,
                              flankSize = flankSize)
...
#' @export
#'
#' @aliases panorama_add_scoreC3p
add_scoreC3p <- function(PANORAMA, group_A, newColName = "auto",
                         onNucs = c("A", "C", "G", "T"), minReads = 30,
                         flankSize = 6, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        DT_A <- add_scoreC_3p(PANORAMA$DATA[[id_A]], minReads = minReads, flankSize = flankSize)
...
#'
#' @examples
add_scoreZ_3p = function(PANORAMA, group_A, newColName = "auto",
                         onNucs = c("A", "C", "G", "T"), minReads = 30,
                         flankSize = 6, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        DT_A <- add_scoreZ_3p_util(PANORAMA$DATA[[id_A]], minReads = minReads,
                                   flankSize = flankSize)
...
#'
#' @examples
add_DR <- function(PANORAMA, group_A, newColName = "auto",
                   onNucs = c("A", "C", "G", "T"), minReads = 30, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        DT_A <- deletion_rate(PANORAMA$DATA[[id_A]], minReads = minReads)
...
#'
#' @examples
add_MR <- function(PANORAMA, group_A, newColName = "auto",
                   onNucs = c("A", "C", "G", "T"), minReads = 30, nCores = 1){
    if(newColName == "auto"){
    ...
    OUT <- parallel::mclapply(mc.cores = nCores, sets, function(iSet){
        id_A <- PANORAMA$META[PANORAMA$META$set == iSet & PANORAMA$META$group == group_A,]$id
        DT_A <- add_diffNucToRefRatio(PANORAMA$DATA[[id_A]], minReads = minReads)
...
detect_2OmeScore <- function(DT_low, DT_high, neighFlankSize = 5, minCov = 50){
    ...
    OUT <- lapply(iGenes, function(iG){
        RES <- data.table::data.table(DTL1[[iG]][,colnames(DTL1[[iG]]) %in% panorama_baseCols, with = FALSE])
...
scoreZ <- function(CHR, d = 6, minReads = 30){
    ...
    for(P in (d+1):(length(CHR)-d)){
        ...
        if(sum(allpos) > minReads){
            score[P] <- (ePos - mean(allpos)) / stats::sd(allpos)
        }else{
            next()
        }
    }
